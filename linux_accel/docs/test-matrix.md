...
| V-06 | OpenStack DNS/gRPC 动态双端缓存 | `bench/openstack_dynamic_cache_campaign.sh` | 五种策略、五类负载各五轮；动态 epoch、两端命中、真实回源及严格清理均可追溯 | 2026-07-30 历史（`.1` 生命周期修订前）：125 轮/625 窗口，DNS/gRPC 均零失败；动态切换后四类可缓存负载 `1.24x-2.13x`，回源降为 0；低命中五轮保持 BYPASS。修订后 P0 smoke：动态三窗口 DNS/gRPC 各 `120/120`，发布 `SERVER_CACHE` epoch `2`；独立 server-cache 窗口 DNS/gRPC 各 `40/40`、后端回源 `0`、两端 server hit 非零，受管 `TCP/50052` 规则已清理。跨主机 map 发布仍为串行实验编排 | 历史性能完成；`.1` P0 smoke 通过；P2 完整矩阵重跑和 P1 原子闭环待完成 |
