...
| --- | --- | --- | --- | --- | --- |
| B-01 | Linux 构建 | `./scripts/build_linux.sh` | 生成 `dns_monitor.bpf.o`、`dns_xdp_monitor.bpf.o`、`grpc_monitor.bpf.o`、`dns_monitor`、`grpc_monitor`、`grpc_fast_cache`、`cachectl` | 2026-08-01 Shuka1 精确候选全量构建通过，Python 回归 215 项通过；BPF verifier、runtime control、策略事务及 DNS/gRPC runtime 五项 root 门禁通过 | 已完成 |
...
| V-07 | OpenStack 数据面 Agent 与 epoch coordinator | `python3 -m unittest tests.test_openstack_dataplane_agent tests.test_openstack_epoch_gate tests.test_openstack_epoch_coordinator`；`bench/openstack_systemd_dynamic_e2e.sh` | Neutron/OVS 精确发现；多实例/多端口 reconcile；进程+pin 健康；迁移冻结；四阶段 epoch；失败强制 BYPASS；只清理自有 hook | 2026-08-01 多宿主补充验收：master/compute2 同时运行 Agent，master 的 client DNS、client gRPC、backend gRPC 三个 map ID 独立；真实 `systemctl stop` 后两端 pin/quiesce/进程/Agent hook 清零且 NetMig `0x65/0x66` 保留。随后单轮动态 DNS/gRPC 各 `40/40`，backend 增量 `0`，两协议 cache hit 各 `+50`，epoch `78 -> 79 -> 80 -> 81`，18 项功能断言、七类清理和 manifest 全通过；`formal_rounds=0`，gRPC 为 Guest 用户态 h2c fast-cache。历史手工往返迁移已通过，但新增 systemd 闭环尚未完成真实双 Compute 自动迁移与故障注入 | 固定主机多宿主 smoke 完成；P1 迁移验收待完成 |
