...
| B-04 | OpenStack campaign 生命周期静态检查 | `bash ./tests/openstack_campaign_lifecycle_test.sh` | 四个 campaign 通过 shell 语法检查，且不含 `killall`、共享 guest 工件或共享 pin 根；动态脚本含唯一 nonce、tap lock 与主机身份校验标记 | 2026-07-30 Git Bash 静态检查通过；发布归档将在 `master` 再执行 | 已完成（静态，不替代真实 E2E 清理证明） |
...
| V-06 | OpenStack DNS/gRPC 动态双端缓存 | `bench/openstack_dynamic_cache_campaign.sh` | 五种策略、五类负载各五轮；动态 epoch、两端命中、真实回源及严格清理均可追溯 | 2026-07-30 历史（`.1` 生命周期修订前）：125 轮/625 窗口，DNS/gRPC 均零失败；动态切换后四类可缓存负载 `1.24x-2.13x`，回源降为 0；低命中五轮保持 BYPASS。跨主机 map 发布为串行实验编排 | 历史性能完成；`.1` 重跑和 P1 原子闭环待完成 |
